...
export default function KanbanEngine({
  ...
  const [mobileStageId, setMobileStageId] = useState('all');
  ...
  return (
    <div className="kanban-engine-outer-wrapper" style={{ width: '100%', overflow: 'visible' }}>
      ...
      {/* MOBILE STAGE TABS BAR (Active only on phone screens <= 768px via CSS) */}
      <div className="kanban-mobile-tabs">
        <button
          type="button"
          onClick={() => setMobileStageId('all')}
          style={{
            padding: '5px 12px',
            borderRadius: '20px',
            fontSize: '11px',
            fontWeight: '700',
            border: mobileStageId === 'all' ? '1px solid #0d9488' : '1px solid #e2e8f0',
            background: mobileStageId === 'all' ? '#0d9488' : '#ffffff',
            color: mobileStageId === 'all' ? '#ffffff' : '#475569',
            whiteSpace: 'nowrap',
            cursor: 'pointer',
            flexShrink: 0
          }}
        ...
        {autoGeneratedColumns.map(col => {
          const isSelected = mobileStageId === col.id;
          return (
            <button
              key={col.id}
              type="button"
              onClick={() => setMobileStageId(col.id)}
              style={{
                padding: '5px 12px',
                borderRadius: '20px',
                fontSize: '11px',
      ...
      <div
        ...
        style={{
          width: '100%',
          overflowX: mobileStageId !== 'all' ? 'hidden' : 'auto',
          overflowY: 'hidden',
      ...
      >
        <div
          style={{
            ...
            width: mobileStageId !== 'all' ? '100%' : 'max-content',
            paddingRight: mobileStageId !== 'all' ? '0' : '40px',
            paddingBottom: '4px'
          }}
        >
          {autoGeneratedColumns
            .filter(col => mobileStageId === 'all' || col.id === mobileStageId)
            .map((col) => (
            <div
              key={col.id}
              className={mobileStageId !== 'all' ? 'kanban-mobile-column-wrapper' : ''}
              style={{
                width: mobileStageId !== 'all' ? '100%' : '320px',
                minWidth: mobileStageId !== 'all' ? '100%' : '320px',
                maxWidth: mobileStageId !== 'all' ? '100%' : '320px',
                flexShrink: 0,
                flexGrow: mobileStageId !== 'all' ? 1 : 0
              }}
            >
              <KanbanColumn
                stage={col}
