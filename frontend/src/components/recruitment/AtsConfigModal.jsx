...
import { Plus, Trash2, Eye, EyeOff, LayoutGrid, List } from 'lucide-react';

/**
 * Admin Configuration Center Modal for Recruitment ATS Module
 * Controls: Forms & Fields, Summary Widgets, Search & Filters, List View, Kanban View, Views
 */
export default function AtsConfigModal({
  isOpen,
  onClose,
  moduleConfig,
  onSaveConfig,
  ...
  const [activeTab, setActiveTab] = useState('fields'); // 'fields' | 'summary' | 'search_filters' | 'columns' | 'kanban' | 'views'
  const [configState, setConfigState] = useState({
    fields: [...(moduleConfig.fields || [])],
    summaryWidgets: [...(moduleConfig.summaryWidgets || [])],
    columns: [...(moduleConfig.columns || [])],
    kanbanFields: { ...(moduleConfig.kanbanFields || { position: true, email: true, phone: true, resume: true }) },
    views: { ...(moduleConfig.views || { availableViews: ['kanban', 'list'], defaultView: 'kanban' }) }
  });
  ...
  // Safe Delete Custom Field with Dependency Check
  ...
  const handleSave = () => {
    ...
    showToast('Recruitment ATS configuration saved successfully!', 'success');
    onClose();
  };
  ...
  return (
    <Modal
      ...
      subtitle="Configuration Center: Forms, Summary, Search, Filters, List, Kanban, and Views."
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>

        {/* Navigation Tabs */}
        <div style={{ display: 'flex', borderBottom: '1px solid #e2e8f0', gap: '4px', overflowX: 'auto' }}>
          {[
            { id: 'fields', label: `Forms & Fields (${configState.fields.length})` },
            { id: 'summary', label: `Summary (${configState.summaryWidgets.filter(w => w.enabled).length})` },
            { id: 'search_filters', label: 'Search & Filters' },
            { id: 'columns', label: `List View (${configState.columns.filter(c => c.visible).length})` },
        ...
        {activeTab === 'fields' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ fontSize: '11px', color: '#64748b', background: '#f8fafc', padding: '10px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
              One Schema drives Add, Edit, and View screens. Toggling Create / Edit / View rules updates forms without code changes.
            </div>
        ...
        {activeTab === 'summary' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ fontSize: '11px', color: '#64748b', background: '#f8fafc', padding: '10px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
              Reorder, enable/disable, or add custom stage-count KPI summary cards.
            </div>
        ...
        {activeTab === 'kanban' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ fontSize: '11px', color: '#64748b', background: '#f8fafc', padding: '10px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
              Select which candidate metadata fields display on Kanban cards. Candidate name remains required.
            </div>
        ...
        {activeTab === 'views' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ fontSize: '11px', color: '#64748b', background: '#f8fafc', padding: '10px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
              Configure supported views for Recruitment ATS and set default view mode on load.
            </div>
