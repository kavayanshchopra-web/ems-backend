import{a as L,n as U,t as H}from"./jsx-runtime-m-EpYe9t.js";import{t as J}from"./bell-BDVhZhkW.js";import{t as F}from"./laptop-DPgvZxTP.js";import{t as K}from"./phone-call-ygSq32CR.js";import{t as Q}from"./plus-CejNS_fS.js";import{B as G,C as Y,g as Z,z as V}from"./index-DKS_aoc4.js";var c=L(U(),1),e=H();function le({authUser:E,sessions:R=[],contacts:X=[],activeContact:f,setActiveContact:ee,setActiveTab:te}){const s=String(E?.tenantId||E?.companyId||typeof window<"u"&&window.__omniflow_tenant||"default_tenant"),[A,N]=(0,c.useState)(()=>{try{const t=localStorage.getItem(`omniflow_custom_staff_${s}`)||localStorage.getItem("omniflow_custom_staff_accounts");if(t)return JSON.parse(t)}catch{}return[{id:"primary",name:"Primary Account",phone:"Scan QR to connect",status:"idle"}]}),[h,z]=(0,c.useState)(()=>localStorage.getItem(`omniflow_selected_staff_id_${s}`)||localStorage.getItem("omniflow_selected_staff_id")||"primary"),[_,P]=(0,c.useState)(()=>{try{const t=localStorage.getItem(`omniflow_staff_unreads_${s}`)||localStorage.getItem("omniflow_staff_unreads");if(t)return JSON.parse(t)}catch{}return{}}),[b,S]=(0,c.useState)(!1),[B,y]=(0,c.useState)(!1),[I,k]=(0,c.useState)(""),[j,C]=(0,c.useState)(Date.now()),[ne,M]=(0,c.useState)(null),l=(0,c.useRef)(null),T=(0,c.useRef)(null),D=(R||[]).filter(t=>t?s!=="1"&&s!=="default_tenant"?String(t.tenant_id||t.tenantId||"")===s:!0:!1),w=D.length>0?D.map((t,n)=>({id:t.id||`staff_${n+1}`,name:t.name||`Staff ${n+1}`,phone:t.phone_number||t.phone||`Account ${n+1}`,status:t.status==="connected"?"connected":"idle"})):A,m=(w||[]).find(t=>t&&t.id===h)||w&&w[0]||null;(0,c.useEffect)(()=>{const t=l.current;if(!t)return;const n=i=>{const r=(i.title||"").match(/^\((\d+)\)/),d=r?parseInt(r[1],10):0;P(p=>{const o={...p,[h]:d};try{localStorage.setItem("omniflow_staff_unreads",JSON.stringify(o))}catch{}return o})};return t.addEventListener("page-title-updated",n),window.__omniflow_send_whatsapp_message=async({phone:i,text:r})=>{const d=String(i||"").replace(/\D/g,"");if(!d||!r)return{success:!1,error:"Phone and text required"};const p=d.length===10?`91${d}`:d,o=l.current;if(!o)return{success:!1,error:"WhatsApp Webview not mounted"};try{if(o.executeJavaScript){const a=`
            (function() {
              try {
          ...
          `,g=await o.executeJavaScript(a);let x=0;const u=setInterval(()=>{x++;try{o.sendInputEvent&&(o.sendInputEvent({type:"keyDown",keyCode:"Return"}),o.sendInputEvent({type:"char",keyCode:"\r"}),o.sendInputEvent({type:"keyUp",keyCode:"Return"}))}catch{}x>15&&clearInterval(u)},350);return{success:!0,result:g}}else if(o.src)return o.src=`https://web.whatsapp.com/send?phone=${p}&text=${encodeURIComponent(r)}`,{success:!0,method:"src_nav"}}catch(a){return{success:!1,error:a.message}}},()=>{t.removeEventListener("page-title-updated",n),delete window.__omniflow_send_whatsapp_message}},[h,j]),(0,c.useEffect)(()=>{const t=l.current;if(!t)return;let n=null;const i=async()=>{try{if(!t.executeJavaScript)return;const d=f?String(f.phone||f.rawPhone||f.phoneNumber||"").replace(/\D/g,""):"",p=f?.name||"",o=`
          (function() {
            try {
