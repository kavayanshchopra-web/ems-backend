...
public class PostCallDispositionActivity extends AppCompatActivity {
    ...
    private void extractIntentData(Intent intent) {
        if (intent != null) {
            ...
            String sSlot = intent.getStringExtra("sim_slot");
            if (sSlot != null && !sSlot.trim().isEmpty()) {
                simSlot = sSlot.trim();
            }
        }
    ...
    private View buildLayout() {
        ...
        bg.setColor(Color.parseColor("#064E43")); // Deep Forest Teal matching CRM brand
        bg.setCornerRadius(24f);
        bg.setStroke(2, Color.parseColor("#0D6D5E"));
        ...
        tvPhoneSub.setText(phoneNumber + " • " + callType + " • " + simSlot);
        ...
        // ==========================================
        // 2. SECTION: LEAD OUTCOME / DISPOSITION CARD
        // ==========================================
    ...
    private LinearLayout createSectionCard(String title) {
        ...
        cBg.setColor(Color.parseColor("#043B33")); // Elevated Card Surface in CRM Teal
        cBg.setCornerRadius(16f);
        cBg.setStroke(1, Color.parseColor("#085F53"));
    ...
    private void openDatePicker() {
        ...
        DatePickerDialog dpd = new DatePickerDialog(this, (view, year, month, dayOfMonth) -> {
            ...
            String displayStr = new SimpleDateFormat("EEE, dd MMM", Locale.getDefault()).format(chosen.getTime());
            String apiDateStr = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(chosen.getTime());
            selectedFollowUpDate = apiDateStr;
            if (tvDatePill != null) {
                tvDatePill.setText(displayStr);
                tvDatePill.setTextColor(Color.WHITE);
                GradientDrawable activeBg = new GradientDrawable();
                activeBg.setColor(Color.parseColor("#064E43"));
                activeBg.setCornerRadius(12f);
                activeBg.setStroke(2, Color.parseColor("#00A8B5"));
                tvDatePill.setBackground(activeBg);
            }
    ...
    private void openTimePicker() {
        ...
        TimePickerDialog tpd = new TimePickerDialog(this, (tView, hourOfDay, minute) -> {
            ...
            if (tvTimePill != null) {
                ...
                tvTimePill.setTextColor(Color.WHITE);
                GradientDrawable activeBg = new GradientDrawable();
                activeBg.setColor(Color.parseColor("#064E43"));
                activeBg.setCornerRadius(12f);
                activeBg.setStroke(2, Color.parseColor("#00A8B5"));
                tvTimePill.setBackground(activeBg);
            }
    ...
    private void updateDispositionStyles() {
        for (int i = 0; i < dispositions.length; i++) {
            ...
            if (isSelected) {
                chipBg.setColor(Color.parseColor("#00A8B5")); // Vibrant Cyan-Teal accent
                chipBg.setStroke(2, Color.WHITE);
                btn.setText("✔ " + dispositions[i]);
                btn.setTextColor(Color.WHITE);
                btn.setTypeface(null, Typeface.BOLD);
            } else {
                chipBg.setColor(Color.parseColor("#05443B")); // Subtle Surface Teal
                chipBg.setStroke(1, Color.parseColor("#085F53"));
                btn.setText(dispositions[i]);
