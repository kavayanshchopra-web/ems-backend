    ...
    public static void syncStage1Instant(
        ...
        new Thread(() -> {
            try {
                ...
                String resolvedAgent = (agentName != null && !agentName.trim().isEmpty()) ? agentName : "Mobile Telecaller";
                ...
                Log.d(TAG, "⚡ [Stage 1 SupabaseSync] Instant sync for: " + targetPhone + " [CallId: " + callId + "]");
                ...
                try {
                    ...
                    callPayload.put("tenant_id", TENANT_ID);
                    ...
                    callPayload.put("notes", "Call completed via " + (simSlot != null ? simSlot : "SIM 1") + " [Ref: " + actualCallId + "]");
                ...
                try {
                    URL checkUrl = new URL(SUPABASE_REST_URL + "/contacts?tenant_id=eq." + TENANT_ID + "&phone_normalized=eq." + norm10);
                    ...
                    if (!leadExists) {
                        ...
                        leadPayload.put("tenant_id", TENANT_ID);
    ...
    public static void syncStage2FollowUp(
        ...
        new Thread(() -> {
            try {
                ...
                Log.d(TAG, "🚀 [Stage 2 SupabaseSync] Follow-up sync for: " + targetPhone + ", Disp: " + resolvedDisp + ", AudioBytes: " + (audioBytes != null ? audioBytes.length : 0));
                ...
                if (audioBytes != null && audioBytes.length > 500) {
                    try {
                        ...
                        String objectPath = "tenants/" + TENANT_ID + "/calls/" + audioFileName;
                ...
                try {
                    ...
                    // Try PATCH by id
                    URL patchUrl = new URL(SUPABASE_REST_URL + "/call_logs?id=eq." + actualCallId + "&tenant_id=eq." + TENANT_ID);
                    ...
                    if (!patched) {
                        try {
                            ...
                            URL ppUrl = new URL(SUPABASE_REST_URL + "/call_logs?customer_phone=eq." + encPhone + "&tenant_id=eq." + TENANT_ID + "&order=created_at.desc&limit=1");
                    ...
                    if (!patched) {
                        ...
                        updatePayload.put("tenant_id", TENANT_ID);
                        updatePayload.put("customer_phone", targetPhone);
                        updatePayload.put("customer_name", resolvedCustName);
                        updatePayload.put("agent_name", (agentName != null && !agentName.isEmpty()) ? agentName : "Mobile Telecaller");
                ...
                try {
                    ...
                    URL leadPatchUrl = new URL(SUPABASE_REST_URL + "/contacts?tenant_id=eq." + TENANT_ID + "&phone_normalized=eq." + norm10);
